...
import type { CMSPage, CMSTeamMember } from '@/hooks/useCMS';
...
const About = () => {
  ...
  const cms = useCMS();
  const { getBlock, isLoading: blocksLoading } = useBlocksByPage('about');
  ...
  const { data: pageContent } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await api.get<CMSPage[]>('/pages', {
        params: { slug: 'about', status: 'published' },
      });
  ...
  // Hero text from CMS
  const heroBadge = pageContent?.hero?.badge ?? 'About Us';
  const heroTitle = pageContent?.hero?.title ?? 'About';
  const heroHighlight = pageContent?.hero?.highlight ?? 'JDGK Business Solutions Inc.';
  const heroDescription = pageContent?.hero?.description ?? 'A dynamic business solutions provider committed to delivering comprehensive and effective strategies tailored to client needs.';

  // CEO message from CMS
  const ceoName = pageContent?.ceo_message?.name ?? 'Donna Bucad Dealca';
  const ceoTitle = pageContent?.ceo_message?.title ?? 'CEO/President';
  const ceoInitials = pageContent?.ceo_message?.initials ?? 'DD';
  const ceoQuotes = pageContent?.ceo_message?.quotes ?? [
  ...
  const missionTitle = pageContent?.mission?.title ?? 'Sustainable Excellence';
  const missionDescription = pageContent?.mission?.description ?? 'To provide sustainable and exceptional business solutions that enable our clients to achieve their strategic goals, optimize operational efficiency, and maximize profitability.';
  const visionTitle = pageContent?.vision?.title ?? 'Leading Provider';
  const visionDescription = pageContent?.vision?.description ?? 'To become a leading provider of comprehensive business solutions, recognized for excellence, innovation, and client success.';

  const timeline = pageContent?.timeline ?? [
  ...
  const ICON_MAP: Record<string, typeof Shield> = { Shield, Award, Users, TrendingUp, Target, Eye };
  ...
  // License images from CMS
  const licenseImages = pageContent?.licenses ?? [
  ...
  const FALLBACK_LEADERSHIP = [
    { name: 'Donna Bucad Dealca', position: 'Chief Executive Officer / President', bio: 'Visionary leader driving strategic direction and business growth with integrity and innovation', avatar: 'DD', slug: 'donna-bucad-dealca' },
    { name: 'Kristofferson Doctor Dealca', position: 'Vice President', bio: 'Dynamic leader focused on operational excellence and business expansion', avatar: 'KD', slug: 'kristofferson-doctor-dealca' },
    { name: 'Jaime Doblado Bucad Jr.', position: 'Board of Directors', bio: 'Expertise in business management and sustainable growth opportunities', avatar: 'JB', slug: 'jaime-doblado-bucad-jr' },
    { name: 'Joan Bucad Landeza', position: 'Board of Directors', bio: 'Supports corporate initiatives and business planning', avatar: 'JL', slug: 'joan-bucad-landeza' },
    { name: 'Erwin Landeza', position: 'Board of Directors', bio: 'Strengthens governance framework and promotes business sustainability', avatar: 'EL', slug: 'erwin-landeza' },
    { name: 'Randy Magauay Rodriguez', position: 'Board of Directors', bio: 'Provides insights in corporate governance and strategic decision-making', avatar: 'RR', slug: 'randy-magauay-rodriguez' },
    { name: 'Von Jaime Horlador Barro', position: 'Board of Directors', bio: 'Shapes company policies and strategies aligned with JDGK mission', avatar: 'VB', slug: 'von-jaime-horlador-barro' },
    { name: 'Geraldine Bucad Barro', position: 'Corporate Secretary', bio: 'Oversees corporate compliance, documentation, and governance', avatar: 'GB', slug: 'geraldine-bucad-barro' },
    { name: 'Zandy Lyn Jesalva Laid', position: 'Admin Head', bio: 'Manages administrative operations and daily business functions', avatar: 'ZL', slug: 'zandy-lyn-jesalva-laid' },
    { name: 'Joshell Tuliao Rodriguez', position: 'Auditor', bio: 'Ensures financial transparency, compliance, and accountability', avatar: 'JR', slug: 'joshell-tuliao-rodriguez' },
  ];
  ...
  // Get data from Block Manager (primary source) or fallback
  const leadershipBlock = getBlock('about-leadership');
  const blockMembers = leadershipBlock?.members as { name: string; role: string; bio: string; avatar: string }[] | undefined;

  const leadership = blockMembers?.length
    ? blockMembers.map(m => ({
      name: m.name,
      position: m.role, // Block uses 'role', UI uses 'position'
      bio: m.bio,
      avatar: getInitials(m.name),
      avatarUrl: m.avatar || null, // Block uses 'avatar' for image URL
      slug: nameToSlug(m.name)
    }))
    : (teamMembersQuery.data && teamMembersQuery.data.length > 0
      ? teamMembersQuery.data
        .filter(m => m.is_leadership) // Only show leadership team
        .map((m) => ({
          name: m.name,
          ...
          slug: m.slug || nameToSlug(m.name),
        }))
      : FALLBACK_LEADERSHIP);

  const finalLoading = blocksLoading || (teamMembersQuery.isLoading && !blockMembers?.length);
          ...
          <div className="container mx-auto px-4 relative z-10">
            <div className="max-w-4xl mx-auto text-center">
              <div className="inline-flex items-center gap-2 bg-primary/10 backdrop-blur-sm border border-primary/20 rounded-full px-6 py-2.5 mb-8">
                ...
                <span className="text-primary text-sm font-semibold tracking-wide">{heroBadge}</span>
              </div>
                ...
                <div className="flex flex-col items-center mb-10">
                  <div className="w-28 h-28 rounded-full bg-gradient-to-br from-primary via-accent to-primary p-1 mb-6 shadow-glow">
                    <div className="w-full h-full rounded-full bg-card flex items-center justify-center">
                      <span className="text-3xl font-bold bg-gradient-to-br from-primary to-accent bg-clip-text text-transparent">{ceoInitials}</span>
                    </div>
                  </div>
                  <h3 className="text-2xl md:text-3xl font-bold mb-1">{ceoName}</h3>
                  <p className="text-primary font-semibold text-lg">{ceoTitle}</p>
                </div>
                <div className="space-y-6 text-foreground/80 text-lg leading-relaxed">
                  {ceoQuotes.map((quote: string, idx: number) => (
                    <p key={idx} className={idx === 0 ? 'italic border-l-4 border-primary pl-6' : ''}>
                      "{quote}"
                    </p>
                  ))}
                </div>
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid lg:grid-cols-2 gap-8 max-w-6xl mx-auto mb-24">
              <div className="group bg-card border-2 border-primary/20 rounded-2xl p-8 md:p-10 hover:border-primary/40 hover:shadow-glow transition-all duration-300">
                ...
                <h3 className="text-2xl md:text-3xl font-bold mb-4">{missionTitle}</h3>
                <p className="text-foreground/80 text-lg leading-relaxed">
                  {missionDescription}
                </p>
              </div>
              <div className="group bg-card border-2 border-accent/20 rounded-2xl p-8 md:p-10 hover:border-accent/40 hover:shadow-glow transition-all duration-300">
                ...
                <h3 className="text-2xl md:text-3xl font-bold mb-4">{visionTitle}</h3>
                <p className="text-foreground/80 text-lg leading-relaxed">
                  {visionDescription}
                </p>
              </div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
              {values.map((value: { icon?: string; title: string; desc: string }, index: number) => {
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 max-w-7xl mx-auto">
              {finalLoading ? (
                Array.from({ length: 8 }).map((_, i) => (
                  <div key={i} className="bg-card border border-border rounded-2xl p-6 h-full">
                    ...
                    <div className="bg-card border border-border rounded-2xl p-6 hover:border-primary/40 hover:shadow-glow transition-all duration-300 h-full">
                      <div className="flex flex-col items-center text-center">
                        <div className="w-20 h-20 mb-4 rounded-full bg-gradient-to-br from-primary to-accent p-1 group-hover:scale-110 transition-transform shadow-soft">
                          {'avatarUrl' in leader && leader.avatarUrl ? (
                            <img src={leader.avatarUrl} alt={leader.name} loading="lazy" className="w-full h-full rounded-full object-cover" />
                          ) : (
                            <div className="w-full h-full rounded-full bg-card flex items-center justify-center">
        ...
        <section className="py-24 bg-gradient-to-b from-muted/30 to-background">
          <div className="container mx-auto px-4">
            ...
            <div className="max-w-5xl mx-auto">
              ...
              >
                <CarouselContent>
                  {licenseImages.map((license: { src: string; alt: string; caption: string }, index: number) => (
