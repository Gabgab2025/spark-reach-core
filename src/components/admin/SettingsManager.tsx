    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure SMTP settings for contact form emails, notifications, and OTP delivery
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="JDGK Business Solutions"
              />
            </div>
          </div>

          <div>
            <Label htmlFor="smtp_recipient_email">Contact Form Recipient Email</Label>
            <Input
              id="smtp_recipient_email"
              type="email"
              value={settings.smtp_recipient_email || ''}
              onChange={(e) => updateSetting('smtp_recipient_email', e.target.value)}
              placeholder="inquiries@jdgkbsi.ph"
            />
            <p className="text-xs text-muted-foreground mt-1">
              Where contact form submissions will be sent. Defaults to the "From" address if empty.
            </p>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">
                  Most providers use STARTTLS on port 587
                </p>
              </div>
              <Switch
          ...
          <div className="flex items-center gap-3">
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  await handleSave();
                  const { data, error } = await cms.getSettings().then(() =>
                    import('@/lib/api').then(m => m.api.post('/settings/test_email', {}))
                  );
                  ...
                  toast({
                    ...
                    description: err?.message || "Could not send test email. Check your SMTP settings.",
                    variant: "destructive",
                  });
                }
              }}
            ...
            <p className="text-sm text-muted-foreground">
              Save settings first, then sends a test email to verify your SMTP configuration
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <Label htmlFor="recaptcha_secret_key" className="text-muted-foreground">Secret Key</Label>
                <p className="text-xs text-muted-foreground mt-1">
                  Managed via server environment variable <code>RECAPTCHA_SECRET_KEY</code>. Not editable from the UI for security.
                </p>
              </div>
            </div>
          </div>
