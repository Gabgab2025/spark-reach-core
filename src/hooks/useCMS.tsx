...
export const useCMS = () => {
  ...
  const getGalleryItems = async (): Promise<CMSGalleryItem[]> => {
  ...
  const createGalleryItem = async (itemData: Partial<CMSGalleryItem>): Promise<void> => {
    const { error } = await api.post('/gallery_items', itemData);

    if (error) throw error;
    toast({ title: "Success", description: "Gallery item added successfully" });
  };

  const updateGalleryItem = async (id: string, itemData: Partial<CMSGalleryItem>): Promise<void> => {
    const { error } = await api.put(`/gallery_items/${id}`, itemData);
  ...
  const deleteGalleryItem = async (id: string): Promise<void> => {
    const { error } = await api.delete(`/gallery_items/${id}`);

    if (error) throw error;
    toast({ title: "Success", description: "Gallery item removed successfully" });
  };
  ...
  const getDashboardStats = async () => {
    try {
      const [pages, services, blogPosts, jobListings, testimonials, teamMembers, galleryItems] = await Promise.all([
        getPages(),
        getServices(),
        getBlogPosts(),
        getJobListings(),
